...
func GenerateOtaPackageVersions(update_request *models.UpdateRequest) string {
	versions := make([]string, 0, 2)
	cps := update_request.Cps
	for _, cp := range cps {
		versions = append(versions, fmt.Sprintf("%s-%s", cp.Mode, cp.Version))
	}
